...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { data: photoPublic } = db.storage.from(BUCKET).getPublicUrl(photoPath)
    const photoUrl = photoPublic.publicUrl
    ...
    // 3. Create D-ID talk
    const expressionCoeff = style === 'lively' ? 1.5 : style === 'soft' ? 0.7 : 1.0
    ...
    const talkRes = await fetch(`${DID_BASE}/talks`, {
      ...
      body: JSON.stringify({
        ...
        config: {
          ...
          fluent: true,
          pad_audio: 0.0,
          expression_scale: expressionCoeff,
        },
      }),
      signal: AbortSignal.timeout(30_000),
    })
    ...
    if (!talkRes.ok) {
      const errBody = await talkRes.text()
      throw new Error(`D-ID talk creation failed: ${talkRes.status} ${errBody}`)
    }

    const talkData: { id: string; status: string } = await talkRes.json()
    const talkId = talkData.id

    // 4. Poll for completion (up to 3 minutes)
    const MAX_POLLS = 36
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      ...
      const pollData: { id: string; status: string; result_url?: string; error?: { description: string } } = await pollRes.json()
      ...
      if (pollData.status === 'error') {
        throw new Error(`D-ID processing error: ${pollData.error?.description ?? 'unknown'}`)
      }
    }
